"""Submit one immutable Word plan and retain its execution facts without replay."""
...
def rejected(request, application, error):
    result = {"type": "task.response", "app": "word", "taskId": None,
...
def execute(request, application, *, timeout=60, executor_factory=None, contracts=None,
    ...
    """Admit one Word Task; factories are internal dependencies, never JSON."""
    if application != "word":
        raise ValueError("This Task client accepts Word requests only")
    try:
        ...
        if source_path is None:
            raise task_request.TaskRequestError("Word Task requires --task-file for submission identity")
...
def _execute(request, plan, result, path, timeout, executor_factory, contracts, progress):
    ...
    try:
        if executor_factory is None:
            from wps_skills.word.windows.task_factory import build_task
            executor_factory = build_task
        ...
        for step, current in zip(plan["steps"], task_request.result_steps(result)):
            ...
            if current is result["document"]:
                from wps_skills.word.task.plan import check_document
                phase = "document"
