...
def initial(request, path, *, plan=None):
    if request.get("app") == "word" and "version" in request:
        return {"type": "task.response", "app": "word", "taskId": path.name,
    ...
    if request["app"] != "word":
        raise ValueError("Only Word Task receipts can be initialized")
    from wps_skills.word.task.plan import compile_request, SAVE_STEP, PDF_STEP
    if plan is None:
