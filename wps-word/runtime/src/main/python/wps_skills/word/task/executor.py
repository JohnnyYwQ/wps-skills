"""The Word Task's private Action execution and resource ownership boundary."""
...
class WordTask:
    def __init__(self, *, task_id, contracts, adapter, coordinator, launcher=None,
        ...
        self._resources = ExecutionResources(
            application="word", error_scope="TASK", contracts=contracts, adapter=adapter, coordinator=coordinator,
            launcher=launcher, action_timeout_seconds=action_timeout_seconds,
            cleanup_timeout_seconds=cleanup_timeout_seconds,
        )
        self._journal.task_event(task_id=task_id, event="task.resources_acquired")

    def execute(self, address, params):
        trace = self._journal.task_action_trace(task_id=self.task_id)
    ...
    def close(self):
        ...
        self._journal.task_event(task_id=self.task_id, event="task.resources_released", cleanup=result)
        return result
