$wordTimingEnabled = [bool](Get-Command Write-WordBridgeTiming -ErrorAction SilentlyContinue)
try {
    while ($null -ne ($line = [Console]::In.ReadLine())) {
        ...
        if ($wordTimingEnabled) { $nativeTimer = [Diagnostics.Stopwatch]::StartNew() }
        try {
            ...
            if ($wordTimingEnabled) {
                $nativeTimer.Stop()
                Write-WordBridgeTiming -RequestId $requestId -TraceId $request.traceId -Operation $request.operation -Phase $nativePhase -DurationMs $nativeTimer.Elapsed.TotalMilliseconds
        ...
        if ($wordTimingEnabled) {
            $nativeTimer.Stop()
            Write-WordBridgeTiming -RequestId $requestId -TraceId $request.traceId -Operation $request.operation -Phase $nativePhase -DurationMs $nativeTimer.Elapsed.TotalMilliseconds -Outcome $response.outcome
            $nativeTimer.Restart()
        }
        try { Write-BridgeRecord -Record $response }
            ...
            if ($wordTimingEnabled) {
                $nativeTimer.Stop()
                Write-WordBridgeTiming -RequestId $requestId -TraceId $request.traceId -Operation $request.operation -Phase 'native.response_write' -DurationMs $nativeTimer.Elapsed.TotalMilliseconds
            }
        }
    }
}
