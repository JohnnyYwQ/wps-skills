...
class WordContractSetTests(unittest.TestCase):
    def test_mutation_results_use_observed_facts_without_changed_count(self):
        for action in ("replaceContent", "setHeaderFooter", "setPageLayout"):
            with self.subTest(action=action):
                result = copy.deepcopy(VALID_RESULTS[action])
                result.pop("changedCount", None)
                WORD_TARGET_CONTRACT_SET.validate_result(
                    action, result, params=VALID_PARAMS[action],
                )
                with self.assertRaises(ContractValidationError):
                    WORD_TARGET_CONTRACT_SET.validate_result(
                        action, dict(result, changedCount=0), params=VALID_PARAMS[action],
                    )

    def test_deletion_still_requires_revision_change_without_changed_count(self):
        params = {
            "target": {"kind": "range", "range": content_range(0, 1)},
            "replacement": {"kind": "delete"},
        }
        result = {"revisionBefore": "revision-1", "revisionAfter": "revision-1",
                  "matchedCount": 1, "ranges": (content_range(0, 0),)}
        with self.assertRaises(ContractValidationError):
            WORD_TARGET_CONTRACT_SET.validate_result("replaceContent", result, params=params)
        result.update(revisionAfter="revision-2", ranges=(content_range(0, 0, revision="revision-2"),))
        WORD_TARGET_CONTRACT_SET.validate_result("replaceContent", result, params=params)

    def test_format_only_replacement_can_preserve_content_revision(self):
        params = {
            "target": {"kind": "query", "expectedMatchCount": 1, "query": {
                "scope": {"kind": "document"}, "text": "A",
                "caseSensitive": True, "wholeWord": False,
            }},
            "replacement": {"kind": "text", "runs": ({"text": "A", "format": {"bold": True}},)},
        }
        result = {"revisionBefore": "revision-1", "revisionAfter": "revision-1",
                  "matchedCount": 1, "ranges": (content_range(0, 1),)}
        WORD_TARGET_CONTRACT_SET.validate_result("replaceContent", result, params=params)
        params["replacement"]["runs"][0]["text"] = "B"
        with self.assertRaises(ContractValidationError):
            WORD_TARGET_CONTRACT_SET.validate_result("replaceContent", result, params=params)
