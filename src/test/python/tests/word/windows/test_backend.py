...
from wps_skills.word.windows.backend import (
    WindowsWordBackend,
    WindowsWordBridge,
)
...
class WindowsWordBackendTests(unittest.TestCase):
    ...
    def test_open_returns_exact_opaque_document_and_closed_observations(self):
        ...
        self.assertIsInstance(acquired.document, WindowsDocument)
    ...
    def test_liveness_and_operations_require_the_exact_bound_document(self):
        ...
        with self.assertRaisesRegex(ValueError, "another document"):
            backend.is_document_live(WindowsDocument(
    ...
    def test_definite_and_unprovable_acquisition_failures_remain_distinct(self):
        definite = BackendActionFailure(
            outcome="failed",
            code="DOCUMENT_NOT_FOUND",
            message="missing",
            binding_disposition="unchanged",
        )
        backend = WindowsWordBackend(bridge=RecordingBridge({
            "acquire_existing_document": definite,
        }))
        with self.assertRaises(WordDefiniteEstablishFailure):
            open_document(backend, "C:/docs/missing.docx")

        unknown = BackendActionFailure(
            outcome="unknown",
            code="RESPONSE_LOST",
            message="lost",
        ...
        with self.assertRaises(WordUnprovableEstablishFailure):
