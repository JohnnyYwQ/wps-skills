...
class WordPowerShellResourceTests(unittest.TestCase):
    def test_production_word_bridge_resource_is_in_main_resource_set(self):
        self.assertTrue(BRIDGE_SCRIPT.is_file())
        self.assertTrue(
            BRIDGE_SCRIPT.with_name("word_actions.ps1").is_file()
        )
    ...
    def test_bridge_requires_and_loads_the_separate_action_resource(self):
        bridge_source = BRIDGE_SCRIPT.read_text(encoding="utf-8")
        actions_path = BRIDGE_SCRIPT.with_name("word_actions.ps1")

        self.assertTrue(actions_path.is_file())
        self.assertIn(
            "$wordActionsPath = Join-Path $PSScriptRoot "
            "'word_actions.ps1'",
            bridge_source,
        )
        self.assertIn(". $wordActionsPath", bridge_source)
    ...
    def test_image_and_section_break_results_use_observed_wps_facts(self):
        actions_source = BRIDGE_SCRIPT.with_name(
            "word_actions.ps1"
