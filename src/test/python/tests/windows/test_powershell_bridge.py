...
class PowerShellBridgeTests(unittest.TestCase):
    def test_transport_satisfies_its_seam(self):
        transport = RecordingTransport({
            "requestId": "<request>",
            "outcome": "succeeded",
            "data": {},
        })
        self.assertIsInstance(transport, BridgeTransport)
    ...
    def test_successful_exchange_uses_controller_correlation_and_deadline(self):
        ...
        bridge = PowerShellBridge(transport=transport)
    ...
    def test_closed_failure_becomes_backend_action_failure(self):
        ...
        with self.assertRaises(BackendActionFailure) as raised:
            bridge.execute(
    ...
    def test_malformed_or_mismatched_response_is_unprovable(self):
        malformed = PowerShellBridge(transport=RecordingTransport({
            "requestId": "<request>",
            "outcome": "succeeded",
            "data": {},
            "unexpected": True,
        }))
        with self.assertRaises(BackendActionFailure) as malformed_error:
        ...
        mismatched = PowerShellBridge(transport=RecordingTransport({
            "requestId": "another-request",
            "outcome": "succeeded",
            "data": {},
        }))
        with self.assertRaises(BackendActionFailure) as mismatch_error:
    ...
    def test_transport_failure_is_unprovable(self):
        class FailedTransport:
            def exchange(self, request, deadline_at):
                raise BridgeTransportError("channel failed")

            def close(self):
                return True

        bridge = PowerShellBridge(transport=FailedTransport())
...
class JsonLineBridgeTransportTests(unittest.TestCase):
    ...
    def test_dead_process_and_expired_deadline_do_not_dispatch(self):
        ...
        transport = JsonLineBridgeTransport(process=dead)
