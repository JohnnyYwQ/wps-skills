...
已有文件使用 `openPresentation`；明确新建时使用 `createPresentation`，不传路径。首次保存和另存为都是同一个 `saveAs` Action，参数为 Windows 主机上的绝对 `.pptx` 路径和 `overwritePolicy: "failIfExists"`。
...
单页图片使用 `exportSlideImage`，传入 `listSlides` 返回的 `slideId`、尚不存在的 `.png` 路径、`overwritePolicy: "failIfExists"`，以及 1–4096 的 `width`、`height` 像素。输出验证 PNG 文件头和实际宽高；尺寸比例应按页面比例选择。

保存／导出观察最多 200 页、每页 100 个顶层形状，沿用 `getSlideInfo` 的文本及属性上限。比较的是这些已支持字段，不宣称验证全部备注、动画、图片字节或表格样式；交付时仍检查实际页面。
