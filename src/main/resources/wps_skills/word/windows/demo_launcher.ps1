# Run the visible Word demo without allocating an auxiliary console.
...
$checkPath = Join-Path $repository 'src/main/resources/wps_skills/word/windows/word_com.ps1'
. ([scriptblock]::Create([IO.File]::ReadAllText($checkPath)))
$availability = Get-WordComAvailability
if (-not $availability.available) {
    if ($availability.reason -eq 'elevated_user_registration') {
        throw '当前是管理员 PowerShell，无法使用这台电脑上仅为当前用户注册的 WPS 文字。请从开始菜单打开普通 PowerShell（不要选择“以管理员身份运行”），再执行同一命令。尚未创建或打开Word 文档。'
    }
    throw $availability.message
}
$entry = Join-Path $repository 'scripts/demo/word.py'
if (-not (Test-Path -LiteralPath $entry -PathType Leaf)) { throw 'Word demo entry was not found in this repository.' }
...
try {
    ...
    if ($process.ExitCode -ne 0) {
        throw ('Word demo failed (exit ' + $process.ExitCode + '): ' + $errorText.Trim())
    }
