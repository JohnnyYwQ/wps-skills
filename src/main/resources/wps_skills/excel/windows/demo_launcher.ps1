# Run the visible Excel demo without allocating an auxiliary console.
...
$checkPath = Join-Path $repository 'src/main/resources/wps_skills/excel/windows/excel_com.ps1'
...
if (-not $availability.available) {
    if ($availability.reason -eq 'elevated_user_registration') {
        throw '当前是管理员 PowerShell，无法使用这台电脑上仅为当前用户注册的 WPS 表格。请从开始菜单打开普通 PowerShell（不要选择“以管理员身份运行”），再执行同一命令。尚未创建或打开演示工作簿。'
    }
    throw $availability.message
}
$entry = Join-Path $repository 'scripts/demo/excel.py'
if (-not (Test-Path -LiteralPath $entry -PathType Leaf)) { throw 'Excel demo entry was not found in this repository.' }
...
try {
    ...
    if ($process.ExitCode -ne 0) {
        throw ('Excel demo failed (exit ' + $process.ExitCode + '): ' + $errorText.Trim())
    }
