...
class WindowsExcelBackend:
    ...
    def prepare_create(self, context):
        ...
        try:
            ...
            _establish_failure(exc)
        self._preparation = ExcelPreparation(_text(result['preparationId']), _text(result['coordinationIdentity']), None)
        return self._preparation
    ...
    def create(self, preparation, context):
        if preparation is not self._preparation or preparation.path is not None or self._document is not None:
            raise ValueError('Excel backend requires its own new-document preparation')
        try:
