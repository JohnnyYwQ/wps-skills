"""Shared Action contracts, validation, and exact-document execution resources."""
...
# Placeholder used only by static plan validation, never by Action dispatch.
DEFERRED_VALUE = object()


def _has_deferred(value):
    if value is DEFERRED_VALUE:
        return True
    if isinstance(value, Mapping):
        return any(_has_deferred(v) for v in value.values())
    if isinstance(value, (list, tuple)):
        return any(_has_deferred(v) for v in value)
    return False
...
def _instance_error(
    ...
    *, partial: bool = False,
    ...
    if partial and value is DEFERRED_VALUE:
        return None
    if "oneOf" in schema:
        matches = [
            branch
        ...
        if not matches or (len(matches) != 1 and not (partial and _has_deferred(value))):
    ...
    if expected_type == "object":
        ...
        if ordered is not None and not (partial and _has_deferred(value)) and all(name in value for name in ordered):
            left, right = ordered
            if value[left] > value[right]:
                return f"{path}.{left} must not exceed {path}.{right}"
        strict_ordered = schema.get("x-strictOrdered")
        if strict_ordered is not None and not (partial and _has_deferred(value)) and all(
        ...
        if unique_by is not None and not (partial and _has_deferred(value)):
            seen = set()
        ...
        if schema.get("uniqueItems") is True and not (partial and _has_deferred(value)):
            for index, item in enumerate(value):
                if any(
        ...
        if schema.get("x-rectangular") is True and value and not (partial and _has_deferred(value)):
            widths = {
                len(row)
                for row in value
        ...
        if schema.get("x-strictlyIncreasing") is True and not (partial and _has_deferred(value)) and any(
...
class ApplicationContractSet:
    ...
    def validate_partial_params(self, action, params):
        """Validate all known schema structure before acquiring execution resources."""
        contract = self.resolve(action)
        if contract is None:
            raise ContractValidationError(action=action, location="params", message="unknown Action")
        error = _instance_error(contract.parameters, params, "params", self._format_validators, partial=True)
        if error is not None:
            raise ContractValidationError(action=action, location="params", message=error)
    ...
    def _establish(self, command):
            ...
            with self._state_lock:
                if self._closing:
                    return self._error_result(
                        outcome="unknown",
                        code="RESPONSE_LOST",
                        message="Resource cleanup began during acquisition",
                        controller_state="broken",
        ...
        if self._is_closing():
            return self._error_result(
                outcome="unknown",
                code="RESPONSE_LOST",
                message="Resource cleanup began during document acquisition",
                controller_state="broken",
                binding_disposition="unprovable",
            )
        with self._state_lock:
        ...
        with self._state_lock:
            if self._closing:
                ...
                return self._error_result(
                    outcome="unknown",
                    code="RESPONSE_LOST",
                    message="Resource cleanup began during Lease commit",
                    controller_state="broken",
                    binding_disposition="unprovable",
                )
            self._lease = lease
    ...
    def close(self):
        with self._close_lock:
            ...
            if release_in_progress:
                ...
                release_context = copy_context()
                worker = threading.Thread(
                    target=lambda: release_context.run(release_resources),
                    daemon=True,
                )
                worker.start()
    ...
    def __init__(
        ...
        *,
        ...
        if contracts.application != application:
            raise ValueError(
                "Contract Set application must match the execution application"
            )
        ...
        if adapter.application != application:
            raise ValueError("Adapter application must match the execution application")
    ...
    def _execute_once(
        ...
        if unavailable:
            return self._error_turn(
                ...
                trace=trace,
                code="RUNTIME_CLOSED",
                message="The execution environment cannot accept another Action",
                continuation="terminate",
            )
        if request.address.app != self._application:
            return self._error_turn(
                request=request,
                trace=trace,
                code=self._error_scope + "_APP_MISMATCH",
                message=(
                    f"The execution application is {self._application}, not "
        ...
        if (
            ...
            return self._error_turn(
                request=request,
                trace=trace,
                code=self._error_scope + "_DOCUMENT_NOT_BOUND",
                message="The execution environment has no bound document",
            )
        if (
            contract.binding_role == "establish"
            ...
            return self._error_turn(
                request=request,
                trace=trace,
                code=self._error_scope + "_DOCUMENT_ALREADY_BOUND",
                message="The execution environment is already bound to a document",
            )
        try:
        ...
        self._cleanup_outcome = CleanupOutcome(
            ...
            error=(
                ...
                if cleanup_succeeded
                    ...
                    code=self._error_scope + "_CLEANUP_INCOMPLETE",
                    message=(
                        "Not all owned document resources were "
                        "confirmed released"
                    ),
                )
            ),
...
class ExecutionResources:
    """Resource aggregate owned by one Task."""
    ...
    def execute(self, request, trace):
        return self._runtime.execute(request, trace)
    ...
    def close(self) -> CleanupOutcome:
        with self._close_lock:
            ...
            def close_adapter():
                try:
                    adapter_cleanup["released"] = self._adapter.close() is True
                except BaseException:
                    adapter_cleanup["released"] = False
            close_context = copy_context()
            worker = threading.Thread(target=lambda: close_context.run(close_adapter), daemon=True)
            worker.start()
            worker.join(timeout=self._cleanup_timeout_seconds)
            processes = ()
            ...
            failed = (runtime_cleanup.outcome == "failed" or launcher_failed
                      or not adapter_cleanup.get("released", False))
            self._cleanup_outcome = CleanupOutcome(
                ...
                error=(
                    ActionError(
                        code=self._error_scope + "_CLEANUP_INCOMPLETE",
                        message=(
                            "Not all owned automation resources "
